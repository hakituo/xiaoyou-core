...
# ASR模型配置：通过 pydantic model 统一加载（P2-2）
try:
    from config.settings_adapters import get_asr_settings

    _asr_settings = get_asr_settings()
    DEFAULT_MODEL_PATH = _asr_settings.model_path
    ASR_MODEL_TYPE = _asr_settings.model_type
except Exception as e:
    logging.warning(f"加载ASR配置失败，使用默认值: {e}")
    DEFAULT_MODEL_PATH = str(get_project_root() / "models" / "faster-whisper")
    ASR_MODEL_TYPE = "faster-whisper"
...
class STTConnector:
    ...
    def __init__(self, model_type: str = None, model_path: str = None):
        # 从环境变量获取配置
        self.temp_audio_dir = os.getenv(
            "TEMP_AUDIO_DIR", str(get_project_root() / "multimodal" / "temp_audio")
        )
        ...
        try:
            ...
            if (
                ...
                self.model_path = str(get_project_root() / p)
        except Exception:
            pass
        self.processor = None
        self.model = None
    ...
    def _load_model(self):
        """根据模型类型加载相应的模型"""
        try:
            if self.model_type.lower() == "faster-whisper":
                # 加载Faster-Whisper模型
                logger.info(f"正在加载Faster-Whisper模型: {self.model_path}")
                self._load_faster_whisper_model()
            elif self.model_type.lower() == "whisper":
                # 加载Whisper模型
    ...
    def _load_faster_whisper_model(self):
        ...
        try:
            ...
            # 8G显存推荐用int8或float16，CPU上必须用int8
            compute_type = "int8" if self.device == "cpu" else "float16"

            self.model = WhisperModel(
                "small",  # 默认使用small模型，平衡速度和准确度
                device=self.device,
                compute_type=compute_type,
                download_root=self.model_path,
            )
            logger.info(f"Faster-Whisper模型加载成功 ({self.device}, {compute_type})")
        except Exception as e:
            logger.error(f"加载Faster-Whisper失败: {e}")
            raise
    ...
    def _load_paraformer_model(self):
        ...
        try:
            ...
            try:
                ...
                # P0-19: 添加 timeout 防止 pip install 网络异常时阻塞 STT 初始化
                ...
                logger.error("安装modelscope超时（180s），请检查网络后手动安装")
                raise
    ...
    async def close(self):
        """清理资源"""
        if self.model_type.lower() == "faster-whisper" and self.model:
            del self.model
            self.model = None
            torch.cuda.empty_cache() if (torch and torch.cuda.is_available()) else None
            logger.info("Faster-Whisper模型已卸载")
        elif self.model_type.lower() == "whisper" and self.model:
            # 释放Whisper模型内存
            del self.model
            self.model = None
            self.processor = None
    ...
    def _transcribe_audio_with_faster_whisper(self, audio_path: str, language: str):
        """使用Faster-Whisper模型转录音频"""
        try:
            # 使用C++ VAD预处理：去除静音段，提升转录速度和准确率
            audio_array = None
        ...
        try:
            ...
            if self.model_type.lower() == "faster-whisper":
                ...
                result = await asyncio.to_thread(
                    self._transcribe_audio_with_faster_whisper,
                    audio_file_path,
                    language,
                )
    ...
    def _transcribe_audio_with_paraformer(self, audio_path: str):
        ...
        try:
            ...
            logger.info(
                f"Paraformer转录结果: {text[:50]}..." if len(text) > 50 else text
            )
            return text
        except Exception as e:
    ...
    async def transcribe_audio_data(
        ...
        **kwargs,
        ...
        try:
            # 生成临时文件名
            temp_filename = (
                f"temp_{hash(audio_data)}_{asyncio.get_event_loop().time()}.{format}"
            )
            temp_file_path = os.path.join(self.temp_audio_dir, temp_filename)

            # 使用asyncio.to_thread隔离文件写入操作
            # P0-20: 使用 with 语句确保文件句柄立即关闭，避免 Windows 文件锁
            # 导致后续 finally 块中的 os.remove 失败
            def _write_temp_file(path: str, data: bytes) -> None:
                with open(path, "wb") as f:
                    f.write(data)

            await asyncio.to_thread(_write_temp_file, temp_file_path, audio_data)

            # 调用文件转录方法
            return await self.transcribe_audio_file(
                temp_file_path, language=language, max_duration=max_duration, **kwargs
            )
    ...
    async def batch_transcribe(
        self, audio_files: list[str], language: str = "auto", concurrency_limit: int = 3
    ) -> list[Dict[str, Any]]:
        """
        批量转录多个音频文件
