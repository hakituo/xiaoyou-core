...
DEFAULT_WEIGHT_CONFIG = {
    ...
    "recency_decay_factor": 0.9,  # 时间衰减因子 (默认)
...
# Work: 中等衰减 (0.95)
# Daily: 较快衰减 (0.85)
# Festival: 慢速衰减 (0.98) - 即使是去年的节日也很珍贵
CATEGORY_DECAY_CONFIG = {
    "learning": 0.99,
    "work": 0.90,  # Work: 稍微加快衰减 (0.95 -> 0.90)
    "daily": 0.70,  # Daily: 快速衰减，减少日常闲聊占用 (0.85 -> 0.70)
    "festival": 0.98,
    "uncategorized": 0.75,  # Uncategorized: 加快衰减 (0.90 -> 0.75)
}
...
class MemoryWeightCalculator:
    ...
    def calculate_initial_weight(
        ...
        if emotions and len(emotions) > 0:
            emotion_bonus = min(
                len(emotions) * self.config["emotion_relevance_bonus"], 3.0
            )
    ...
    def apply_time_decay(
        self, weight: float, timestamp: float, category: str = None
    ) -> float:
        """
        应用时间衰减因子
        ...
        # 确定衰减因子
        decay_rate = self.config["recency_decay_factor"]
        if category and category in CATEGORY_DECAY_CONFIG:
            decay_rate = CATEGORY_DECAY_CONFIG[category]

        # 应用衰减因子（每天衰减）
        decay_factor = decay_rate**days_passed
    ...
    def update_weight_by_access(
        self, current_weight: float, importance: int = 1
    ) -> float:
        """
        基于访问更新权重
