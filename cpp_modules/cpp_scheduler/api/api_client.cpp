...
ClientResponse APIClient::generateLLM(const std::string& prompt, const std::string& model, float temperature) {
    ClientRequest request(RequestMethod::POST, "/api/v1/llm/generate");
    request.body = buildLLMRequestBody(prompt);
    return sendRequest(request);
}

void APIClient::generateLLMAsync(const std::string& prompt, APICallback callback, const std::string& model, float temperature) {
    ClientRequest request(RequestMethod::POST, "/api/v1/llm/generate");
    request.body = buildLLMRequestBody(prompt);
    sendRequestAsync(request, callback);
}

ClientResponse APIClient::synthesizeTTS(const std::string& text, const std::string& voice_id, float speed) {
    ClientRequest request(RequestMethod::POST, "/api/v1/tts/synthesize");
    request.body = buildTTSRequestBody(text, voice_id);
    return sendRequest(request);
}

void APIClient::synthesizeTTSAsync(const std::string& text, APICallback callback, const std::string& voice_id, float speed) {
    ClientRequest request(RequestMethod::POST, "/api/v1/tts/synthesize");
    request.body = buildTTSRequestBody(text, voice_id);
    sendRequestAsync(request, callback);
}

ClientResponse APIClient::generateImage(const std::string& prompt, int width, int height, bool use_turbo, int steps) {
    ClientRequest request(RequestMethod::POST, "/api/v1/image/generate");
    request.body = buildImageRequestBody(prompt, width, height);
    return sendRequest(request);
}

void APIClient::generateImageAsync(const std::string& prompt, APICallback callback, int width, int height, bool use_turbo, int steps) {
    ClientRequest request(RequestMethod::POST, "/api/v1/image/generate");
    request.body = buildImageRequestBody(prompt, width, height);
    sendRequestAsync(request, callback);
}

ClientResponse APIClient::getSystemStatus() {
    ClientRequest request(RequestMethod::GET, "/api/v1/status");
    return sendRequest(request);
}

ClientResponse APIClient::cancelTask(const std::string& task_id) {
    ClientRequest request(RequestMethod::DELETE_, "/api/v1/tasks/" + task_id);
    return sendRequest(request);
}

ClientResponse APIClient::getTaskStatus(const std::string& task_id) {
    ClientRequest request(RequestMethod::GET, "/api/v1/tasks/" + task_id);
    return sendRequest(request);
}

ClientResponse APIClient::getResourceStats() {
    ClientRequest request(RequestMethod::GET, "/api/v1/resources");
    return sendRequest(request);
}

ClientResponse APIClient::getImageProgress(const std::string& task_id) {
    ClientRequest request(RequestMethod::GET, "/api/v1/image/" + task_id + "/progress");
    return sendRequest(request);
}
...
void runAPIClientExample() {
    ...
    ClientResponse status_response = client->getSystemStatus();
