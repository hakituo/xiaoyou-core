...
private:
    ...
    std::string api_key_;
    int timeout_ms_;
    void* http_client_handle_;
    
    // 辅助方法
    std::string buildURL(const std::string& endpoint);
    ClientResponse executeRequest(const ClientRequest& request);
    
    std::string buildLLMRequestBody(const std::string& prompt);
    std::string buildTTSRequestBody(const std::string& text, const std::string& voice_id);
    std::string buildImageRequestBody(const std::string& prompt, int width, int height);
};

// JSON辅助函数
std::string toJSON(const ClientResponse& response);
