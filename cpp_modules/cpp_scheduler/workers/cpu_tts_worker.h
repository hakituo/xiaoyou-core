...
public:
    ...
    // Must implement pure virtual methods from ITask
    void execute() override {
        // Implementation provided by worker or placeholder
    }
    ...
    void setCallback(std::function<void(std::shared_ptr<TTSTask>)> callback) {
        callback_ = callback;
    }

    void runCallback(std::shared_ptr<TTSTask> self) {
        if (callback_) {
            callback_(self);
        }
    }
...
private:
    ...
    std::function<void(std::shared_ptr<TTSTask>)> callback_;
};
...
public:
    ...
    void synthesize(const TTSParams& params, TaskPriority priority, 
                   std::function<void(std::shared_ptr<TTSTask>)> callback = nullptr);

    // 设置任务完成回调
    using TaskCompletedCallback = std::function<void(std::shared_ptr<ITask>)>;
