...
public:
    ...
    static bool assertEquals(const std::string& testName, bool expected, bool actual) {
        if (expected != actual) {
            log("[FAIL] " + testName + " FAILED: expected " + std::to_string(expected) + ", got " + std::to_string(actual));
            return false;
        }
        log("[PASS] " + testName + " PASSED");
        return true;
    }
...
public:
    ...
    // 测试4: 图像生成Worker功能
    // 注：C++ 端的图像 worker 已移除（mock 实现），图像生成实际由 Python 侧负责。
    //     此处保留空壳以维持测试套件编号一致，实际不再执行任何 worker 检查。
    static bool testImageWorker() {
        TestUtils::log("测试4: 图像生成Worker功能（已跳过：image worker 已迁移至 Python 侧）");
        return true;
    }
