...
bool SystemIntegrationTest::initialize() {
    ...
    try {
        ...
        // 创建图像生成任务队列
        // 注：图像生成 worker 已迁移至 Python 侧实现，C++ 端仅保留任务队列结构用于调度器兼容。
...
void SystemIntegrationTest::printTestReport(const TestResult& result) const {
    ...
    std::cout << "  Result: " << (result.success ? "[PASS] PASSED" : "[FAIL] FAILED") << std::endl;
...
bool SystemIntegrationTest::checkSystemHealth() {
    // 检查所有组件状态
    // 注：image worker 已迁移至 Python 侧，C++ 端不再检查图像 worker 状态
    ...
    return scheduler_healthy && llm_healthy && tts_healthy;
}
