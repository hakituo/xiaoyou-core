...
public:
    ...
    // 运行所有测试
    bool runAll() {
        std::cout << "\n=== Test Suite Start: " << name_ << " ===" << std::endl;
        ...
        for (auto& test : tests_) {
            ...
            switch (result) {
                case TestResult::SUCCESS: successCount++; break;
                case TestResult::FAILED: failCount++; break;
                case TestResult::SKIPPED: skipCount++; break;
            }
        }
        
        std::cout << "\n=== Test Suite Done: " << name_ << " ===" << std::endl;
...
protected:
    bool execute() override {
        std::cout << "  Test1: scheduler init" << std::endl;
        auto scheduler = std::make_shared<ResourceIsolationScheduler>();
        bool initResult = scheduler->initialize(4);
        
        if (!expectTrue(initResult, "scheduler init failed")) {
            return false;
        }
        
        std::cout << "  Test2: submit different task types" << std::endl;
        ...
        if (!expectTrue(llmResult == "llm ok", "llm result mismatch")) {
            return false;
        }
        
        if (!expectTrue(ttsResult == "tts ok", "tts result mismatch")) {
            return false;
        }
        
        if (!expectTrue(imgResult == "image ok", "image result mismatch")) {
            return false;
        }
        
        std::cout << "  Test3: concurrent tasks" << std::endl;
        ...
        // 验证结果（0+1+2+...+7=28）
        if (!expectTrue(sum == 28, "concurrent result mismatch")) {
            return false;
        }
...
protected:
    bool execute() override {
        std::cout << "  Test1: parallel tasks across resource domains" << std::endl;
        ...
        if (!expectTrue(shortResult == "short task done", "resource isolation failed: short task blocked")) {
            return false;
        }
...
protected:
    bool execute() override {
        std::cout << "  Test1: task priority" << std::endl;
        ...
        if (!expectTrue(highResult == "high priority done", "priority scheduling failed")) {
            return false;
        }
        ...
        // 低优先级任务应该还在运行
        if (!expectTrue(!isLowReady, "low priority task should not be ready")) {
            return false;
        }
...
protected:
    bool execute() override {
        std::cout << "  Test1: create blackbox service" << std::endl;
        ...
        std::cout << "  Note: skipping full integration test (run in real env)" << std::endl;
...
inline bool runAllIntegrationTests() {
    TestSuite suite("Resource Isolation Scheduler Integration Tests");
...
inline int testMain(int argc, char** argv) {
    std::cout << "\n============================================" << std::endl;
    std::cout << "     Resource Isolation Scheduler Test Suite" << std::endl;
    std::cout << "============================================" << std::endl;
    
    bool allPassed = runAllIntegrationTests();
    
    std::cout << "\n============================================" << std::endl;
    std::cout << "Result: " << (allPassed ? "ALL PASSED" : "FAILED") << std::endl;
    std::cout << "============================================" << std::endl;
