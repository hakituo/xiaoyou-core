...
import { formatChatTime } from '../utils/text';
  ...
  const handleCreateClick = async () => {
      ...
      try {
          ...
          if (onRequestConfirm) {
              onRequestConfirm({
                  title: 'Error',
                  message: 'Failed to create new topic. Please check backend logs.',
                  type: 'danger',
                  showCancel: false,
                  onConfirm: () => {}
              });
    ...
    <div className="flex flex-col h-full w-full bg-transparent border-r-0 border-white/5">
      <div className="p-2 border-b border-white/5">
        <button 
          ...
          disabled={isCreating}
          className="w-full flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 text-white/90 border border-white/10 hover:border-white/20 rounded-lg py-2 px-3 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isCreating ? (
