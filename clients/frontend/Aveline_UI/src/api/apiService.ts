...
export const api = {
  ...
  getReferenceAudio: async () => {
    const res = await get('/api/v1/voice/reference-audio', undefined, { retries: 2 });
    return res;
  },
  ...
  sendMessage: async (message: string, options?: {
    ...
    logger.logApiResponse('/api/v1/message', 'POST', requestId, 200, duration, {
      ...
      hasConversationId: !!processedResponse.conversation_id
        ...
        // 获取当前选中的参考音频
    const refAudioPath = sessionStorage.getItem('selected_ref_audio');
    
    // 如果有refAudioPath，添加到TTS请求中
    if (processedResponse.tts_suggest) {
       // 这里我们通过修改tts_suggest或者在后续处理TTS时注入
       // 但sendMessage主要是处理文本回复。真正的TTS调用通常在MemoryPanel或ChatPanel中处理
       // 不过，如果是后端直接返回音频流，这里可能需要传参
    }
  ...
  getImageModels: async (options?: { silent?: boolean }) => {
    const res = await get('/api/v1/image/models', undefined, options);
    return res;
  },
  ...
  clearWeightedMemories: async (userId: string = "default") => {
    const res = await del(`/api/v1/memory/weighted?user_id=${userId}`);
    return res;
  },

  getWeightedMemories: async (limit: number = 100, minWeight: number = 1.0) => {
    const res = await get('/api/v1/memory/weighted', { limit, min_weight: minWeight });
    return res;
  },
  ...
  getSessionHistory: async (sessionId: string) => {
    const res = await get(`/api/v1/sessions/${sessionId}/history`);
    return res;
  },
