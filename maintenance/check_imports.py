                        ...
                        if isinstance(node, ast.Import):
                            for alias in node.names:
                                if alias.name in moved_modules:
                                    print(
                                        f"[WARN] {file_path}:{node.lineno} - Import '{alias.name}' might be deprecated. Consider using '{moved_modules[alias.name]}'."
                                    )
                        elif isinstance(node, ast.ImportFrom):
                            if node.module and node.module in moved_modules:
                                print(
                                    f"[WARN] {file_path}:{node.lineno} - From import '{node.module}' might be deprecated. Consider using '{moved_modules[node.module]}'."
                                )
...
if __name__ == "__main__":
    check_imports(str(Path(__file__).resolve().parents[1]))
