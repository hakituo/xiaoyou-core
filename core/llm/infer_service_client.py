...
class InferServiceClient:
    ...
    def __init__(self, base_url: Optional[str] = None, timeout: int = 300):
        ...
        if base_url is None:
            ...
            host = os.getenv("INFER_SERVICE_HOST", settings.server.host)
            port = int(os.getenv("INFER_SERVICE_PORT", str(settings.server.port)))
    ...
    async def _request_with_retry(
        ...
        for attempt in range(self.retry_count + 1):
            try:
                ...
                last_error = str(e)
                logger.warning(
                    f"请求尝试 {attempt + 1}/{self.retry_count + 1} 失败: {last_error}"
                )
    ...
    async def generate(
        ...
        try:
            ...
            logger.info(
                f"生成请求完成，耗时: {processing_time:.2f}秒，响应长度: {len(result.get('text', ''))} 字符"
            )
    ...
    async with InferServiceClient() as client:
        try:
            ...
            result = await client.generate(
                prompt="你好，请介绍一下你自己", max_tokens=200, temperature=0.7
            )
