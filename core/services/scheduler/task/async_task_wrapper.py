...
class TaskType(Enum):
    """增强任务类型枚举（扩展自调度器TaskType）"""

    CPU = SchedulerTaskType.CPU_BOUND.value
    GPU = SchedulerTaskType.GPU_BOUND.value
    IO = SchedulerTaskType.DEFAULT.value
    TTS = "tts"
    STT = "stt"
    IMAGE = "image"
    LLM = "llm"
...
class AsyncTaskWrapper:
        ...
        **kwargs,
        ...
        async def tts_task_wrapper():
            ...
            try:
                ...
                return {"audio_data": audio_data, "text": text, "language": language}
        ...
        **kwargs,
        ...
        async def stt_task_wrapper():
            ...
            try:
                ...
                return {"text": result, "audio_file": audio_file}
            except Exception as e:
                if task_id:
        ...
        task_id = await self._submit_task(
            ...
            priority=priority,
            args=(),
            metadata={"audio_file": audio_file, "language": language},
        )
    ...
    async def _submit_task(
        ...
        task_id = await self._scheduler.schedule_task(
            func=func, name=name, priority=priority, args=args, kwargs=kwargs or {}
        )
        ...
        if base_info:
            ...
            logger.info(
                f"任务已提交 - ID: {task_id}, 类型: {task_type.name}, 名称: {name}"
            )
    ...
    async def wait_for_task(self, task_id: str, timeout: Optional[float] = None) -> Any:
        ...
        Raises:
            ...
            Exception: 任务执行失败
        """
        # P1-1: 使用 time.monotonic 替代 asyncio.get_event_loop().time()
        # 避免在 Python 3.10+ 触发弃用警告，且 time.monotonic 性能更好
    ...
    def register_progress_callback(
        self, task_id: str, callback: Callable[[str, float, Optional[str]], None]
    ) -> None:
        """
        注册进度回调
...
async def submit_tts_task(
    ...
    **kwargs,
    ...
    wrapper = get_global_task_wrapper()
    return await wrapper.submit_tts_task(
        text=text,
...
async def submit_stt_task(
    ...
    **kwargs,
    ...
    wrapper = get_global_task_wrapper()
    return await wrapper.submit_stt_task(
        audio_file=audio_file, language=language, priority=priority, **kwargs
    )


async def get_task_info(task_id: str) -> Optional[EnhancedTaskInfo]:
    return await get_global_task_wrapper().get_task_info(task_id)


async def cancel_task(task_id: str) -> bool:
    return await get_global_task_wrapper().cancel_task(task_id)


async def wait_for_task(task_id: str, timeout: Optional[float] = None) -> Any:
    return await get_global_task_wrapper().wait_for_task(task_id, timeout)
