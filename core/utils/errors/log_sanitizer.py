...
SENSITIVE_PATTERNS = {
    ...
    # API Key/Secret
    "api_key": re.compile(
        r"(?:api[_-]?key|apikey|secret)[_=:\s]*(?P<key>[a-zA-Z0-9]{20,})"
    ),
    # 密码
...
# P1-2: 跟踪 fire-and-forget 错误上报任务，防止被 GC 后异常被事件循环静默吞掉
_pending_report_tasks: set = set()


def _spawn_error_report(coro) -> None:
    """P1-2: 提交错误上报任务并保存引用，完成后自动清理并记录失败（避免错误上报本身静默失败）。"""
    try:
        task = asyncio.create_task(coro)
    except RuntimeError:
        # 无运行事件循环，直接记录到 logger 避免完全静默
        logger.error("无法报告错误（无运行事件循环）")
        return
    _pending_report_tasks.add(task)

    def _on_done(t: asyncio.Task) -> None:
        _pending_report_tasks.discard(t)
        if t.cancelled():
            return
        exc = t.exception()
        if exc is not None:
            # 错误上报本身失败时打印到 stderr 作为最后兜底
            import sys
            print(f"[ErrorReporter] 上报任务异常: {exc!r}", file=sys.stderr)
...
class LogSanitizer:
    ...
    def sanitize_message(
        ...
        extra_rules: Optional[Dict[str, Callable[[str], str]]] = None,
    ) -> str:
        """
        脱敏日志消息
    ...
    def sanitize_dict(
        ...
        for key, value in data.items():
            ...
            # 检查是否为敏感键
            is_sensitive = any(
                sensitive in key_lower for sensitive in default_sensitive_keys
            )
            ...
            if is_sensitive and isinstance(value, str):
                ...
                # 处理列表
                result[key] = [
                    LogSanitizer.sanitize_dict(item, sensitive_keys)
...
class ErrorReporter:
    ...
    async def report_error(
        ...
        try:
            ...
            await event_bus.publish(
                EventTypes.ERROR_OCCURRED, error_id=error_id, error=error_report
            )
    ...
    async def _process_error_queue():
        ...
        while True:
            try:
                ...
                # 等待一段时间再处理下一批
                await asyncio.sleep(
                    _sanitizer_config["error_reporting"]["report_interval"]
                )
    ...
    async def _save_errors_to_file(errors: List[Dict[str, Any]]):
        ...
        try:
            ...
            # 同时保存到 ErrorLogStore（按日期组织的 error.log）
            try:
                from core.services.error_log_store import get_error_log_store
                error_store = get_error_log_store()
                for err in errors:
                    error_store.append_error(
                        conversation_id=err.get("context", {}).get("path", "system"),
                        user_message=err.get("context", {}).get("method", ""),
                        error_message=err.get("error_message", ""),
                        error_code=err.get("error_type", "UNKNOWN_ERROR"),
                        error_details=err.get("context", {}),
                        stack_trace=err.get("traceback", ""),
                        source="log_sanitizer",
                    )
                logger.info(f"已同步 {len(errors)} 个错误到 ErrorLogStore")
            except Exception as e:
                logger.warning(f"同步到 ErrorLogStore 失败: {e}")
...
def dispatch_error_to_callbacks(error_id: str, error_report: Dict[str, Any]) -> None:
    """同步派发错误报告给所有已注册的回调。

    供 utils/logger.py 的 AutoHealErrorHandler 调用，避免 utils 反向依赖 services 层。
    回调本身可能抛异常，这里逐个捕获并打印，避免影响其它回调或日志流程。
    """
    with _lock:
    ...
    for callback in callbacks:
        try:
            ...
            # 回调失败不能影响日志流程，直接打印到 stderr 作为最后兜底
            import sys
            print(f"[log_sanitizer] 错误回调执行失败: {e!r}", file=sys.stderr)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # P1-2: 保存任务引用，避免错误上报本身被 GC 静默吞掉
                    _spawn_error_report(ErrorReporter.report_error(e, context=context))
                except RuntimeError:
                    # 无运行的事件循环，直接记录错误，不再 fallback 到 run_until_complete
                    logger.error(f"无法报告错误（无运行事件循环）: {e}")
            except Exception:
                # 如果无法获取事件循环，记录但不崩溃
