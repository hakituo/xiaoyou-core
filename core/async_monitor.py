...
class PerformanceMonitor:
    ...
    def _lazy_check_gpu(self):
        ...
        try:
            ...
            logger.info("pynvml initialized successfully (lazy)")
            ...
            for metric_name in [
                ...
                if metric_name in self.current_metrics:
                    self.metrics_history[metric_name].append(
                        (timestamp, self.current_metrics[metric_name])
                    )
        except Exception as e:
    ...
    def _check_health_thresholds(self):
        ...
        if self.current_metrics["cpu_usage"] > cpu_threshold:
            logger.warning(
                f"CPU使用率过高: {self.current_metrics['cpu_usage']}% (阈值: {cpu_threshold}%)"
            )
        ...
        if self.current_metrics["memory_usage"] > memory_threshold:
            logger.warning(
                f"内存使用率过高: {self.current_metrics['memory_usage']}% (阈值: {memory_threshold}%)"
            )
        ...
        if self.current_metrics["async_tasks"] > tasks_threshold:
            logger.warning(
                f"异步任务数量过多: {self.current_metrics['async_tasks']} (阈值: {tasks_threshold})"
            )
...
class HealthChecker:
    ...
    def register_health_checker(
        self, service_name: str, checker_func, interval: float = 30.0
    ):
        """
        注册服务健康检查函数
        """
    ...
    async def check_service_health(self, service_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if raw == HealthStatus.HEALTHY.value:
                ...
                st = HealthStatus(raw)  # type: ignore[arg-type]
            elif raw:
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        获取整体健康状态摘要
        """
        statuses = [str(s.get("status", "") or "") for s in self.health_status.values()]
        all_healthy = all(st == HealthStatus.HEALTHY.value for st in statuses)
        any_error = any(st == HealthStatus.ERROR.value for st in statuses)

        overall = (
            HealthStatus.HEALTHY.value
            if all_healthy
            else (HealthStatus.ERROR.value if any_error else HealthStatus.DEGRADED.value)
        )
        summary = {
...
async def initialize_monitoring(config: Dict[str, Any] = None):
    ...
    if _performance_monitor is not None:
        ...
        if (
            not _performance_monitor.monitor_thread
            or not _performance_monitor.monitor_thread.is_alive()
        ):
    ...
    try:
        ...
        try:
            ...
            if "cpu_usage" not in thresholds:
                thresholds["cpu_usage"] = float(settings.monitor.cpu_threshold_high)
            if "memory_usage" not in thresholds:
                thresholds["memory_usage"] = float(
                    settings.monitor.memory_threshold_high
                )
        except Exception:
            pass

        if "async_tasks" not in thresholds:
            thresholds["async_tasks"] = 1000

        monitor_config["thresholds"] = thresholds
...
async def request_performance_middleware(request, call_next):
    ...
    try:
        ...
        process_time = time.time() - start_time
        logger.error(
            f"请求处理异常: {request.url.path} 耗时: {process_time:.2f}秒, 错误: {e}"
        )
        raise
...
def enhanced_logger(func):
    ...
    async def async_wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.debug(
                f"[完成] {func_name} 耗时: {exec_time:.4f}秒 返回: {type(result).__name__}"
            )
