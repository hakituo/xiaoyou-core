...
def ensure_dir_exists(dir_path: str) -> None:
    """
    确保目录存在，不存在则创建（委托给 core.utils.common.ensure_directory）
    """
    from core.utils.common import ensure_directory
    ensure_directory(dir_path)


def random_sample(
    items: List[Any], sample_size: int, avoid_duplicates: bool = True
) -> List[Any]:
    """
    随机抽取样本
...
def sort_dict_by_value(
    d: Dict[Any, Any], reverse: bool = False
) -> List[Tuple[Any, Any]]:
    """
    按值排序字典
...
def filter_list_by_keywords(
    items: List[Dict[str, Any]], keywords: List[str], fields: List[str]
) -> List[Dict[str, Any]]:
    """
    根据关键词过滤列表
