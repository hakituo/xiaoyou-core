...
class WebSocketClient:
    ...
    def __init__(
        self,
        env_id: str,
        server_url: str = "ws://localhost:8010",
        ...
        self.message_lock = LazyAsyncLock()  # P2-8: 改用 LazyAsyncLock 避免在 __init__ 中创建 asyncio.Lock（无事件循环时会出错）
    ...
    async def connect(self) -> bool:
        ...
        try:
            self.websocket = await websockets.connect(
                ...
                extra_headers={"X-Environment-ID": self.env_id},
            )
        ...
        while self.connected:
            try:
                ...
                if isinstance(raw, dict):
                    ...
                    if t == "message":
                        ...
                        if subtype == "acknowledged":
                            logger.info(
                                f"消息已确认: message_id={raw.get('message_id')}"
                            )
                ...
                if message.type in ("connection_ack", "connection_established"):
                    self.client_id = (
                        message.data.get("client_id")
                        if isinstance(message.data, dict)
                        else None
                    )
                    logger.info(
                        f"连接已建立，客户端ID: {self.client_id or self.env_id}"
                    )
    ...
    async def _handle_reconnect(self):
        ...
        while not self.connected and not self.closing:
            logger.info(
                f"尝试重新连接WebSocket服务器，{self.reconnect_interval}秒后..."
            )
    ...
    async def send_message(
        self, target_env: str, topic: str, data: dict = None
    ) -> bool:
        """发送通用消息（与当前服务路由兼容）"""
        content = (
            ""
            if not data
            else (data.get("content") if isinstance(data, dict) else str(data))
        )
        payload = {
            "type": "message",
    ...
    async def send_ping(self) -> bool:
        """发送ping消息"""
        message = WebSocketMessage(type="ping", timestamp=time.time())
        return await self.send(message)

    async def request_status(self) -> Optional[dict]:
        """请求服务器状态"""
        message = WebSocketMessage(type="get_status")
...
async def get_websocket_client(
    env_id: str, server_url: str = "ws://localhost:8010"
) -> WebSocketClient:
    """
    获取全局WebSocket客户端实例
...
async def send_websocket_message(
    env_id: str, target_env: str, topic: str, data: dict = None
) -> bool:
    """发送WebSocket消息"""
...
def _sync_close_websocket():
    ...
    if loop is not None and loop.is_running():
        loop.call_soon_threadsafe(lambda: asyncio.create_task(close_websocket_client()))
