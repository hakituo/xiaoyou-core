...
class MessageQueue:
    ...
    def put(self, message: Message) -> None:
        ...
        之前直接调用 queue.put() 会抛 AttributeError，因为 MessageQueue
        本身没有 put 方法（内部 self.queue 才是 queue.Queue）。
        """
        with self.lock:
...
class FileBasedSharedStorage:
    ...
    def __init__(self, storage_dir: str = None):
        ...
        self.storage_dir = storage_dir or os.path.join(
            tempfile.gettempdir(), "xiaoyou_shared"
        )
    ...
    def list_keys(self, env_id: str = "global") -> List[str]:
        """列出指定环境下的所有 key。

        之前 check_messages_from_shared_storage 调用了不存在的 list_keys()，
        导致 RuntimeError。这里通过扫描 env 目录下的 .json 文件名返回 key 列表。
        """
        env_dir = os.path.join(self.storage_dir, env_id)
        ...
        try:
            ...
            logger.error(f"列出共享数据 key 失败: {e}")
            return []
        return keys
...
class EnvironmentCommunicationManager:
    ...
    def start(self):
        ...
        self._process_thread = threading.Thread(
            target=self._process_messages_loop, daemon=True
        )
    ...
    async def _forward_via_shared_storage(self, message: Message):
        ...
        try:
            ...
            logger.debug(
                f"通过共享存储转发消息: {message.message_id} 到 {message.target}"
            )
        except Exception as e:
        ...
        try:
            ...
            for key in keys:
                if key.startswith("msg_"):
                    # 读取消息
                    message_data = await self.shared_storage.read(
                        key, env_id=self.env_id
                    )
                    ...
                    if message_data and message_data.get("status") == "pending":
                        ...
                        await self.shared_storage.write(
                            key, message_data, env_id=self.env_id
                        )
...
def cleanup_communication_manager():
    ...
    if _communication_manager:
        try:
            ...
            if (
                _communication_manager._process_thread
                and _communication_manager._process_thread.is_alive()
            ):
                # 发送终止信号
                ...
                if loop is not None and loop.is_running():
                    loop.call_soon_threadsafe(
                        lambda: asyncio.create_task(
                            _communication_manager._close_all_http_clients()
                        )
                    )
            except Exception:
                pass
