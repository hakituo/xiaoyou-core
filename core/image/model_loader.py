...
# torch 和 diffusers 延迟导入，避免启动时加载重型依赖（~3秒）
# import torch
...
CHECKPOINT_DIR = str(MODELS_DIR / "Image" / "check_point")
CHECKPOINT_DIR_OLD = str(MODELS_DIR / "check_point")
CHECKPOINT_DIR_NEW = str(MODELS_DIR / "Image" / "sd1.5" / "check_point")
SDXL_CHECKPOINT_DIR = str(
    MODELS_DIR
    / "Image"
...
ARCHIVE_DIR = str(MODELS_DIR / "archive")
...
class ModelDiscovery:
    ...
    def resolve_model_path(model_id: str) -> Optional[str]:
        ...
        search_paths = [
            ...
            os.path.join(CHECKPOINT_DIR_OLD, model_id),
            os.path.join(ARCHIVE_DIR, model_id),
            str(MODELS_DIR / model_id),
            str(MODELS_DIR / "Image" / model_id),
            str(MODELS_DIR / "img" / model_id),
        ]
        ...
        # 4. 处理常见的拼写错误 (sensitve -> sensitive)
        if "sensitve" in model_id.lower():
            corrected_id = model_id.replace("sensitve", "sensitive", 1)
    ...
    def discover_local_models() -> List[str]:
        ...
        img_models_dir = str(MODELS_DIR / "img")
        model_paths = []
                ...
                for root, _, files in os.walk(search_dir):
                    for f in files:
                        if f.endswith(".safetensors") or f.endswith(".ckpt"):
                            ...
                            # 检查并添加拼写错误的版本
                            if "sensitve" in f.lower():
                                corrected_file = f.replace("sensitve", "sensitive", 1)
...
class ModelLoader:
    ...
    @staticmethod
    def load_pipeline(
        model_id: str, model_type: str = "diffusion", low_cpu_mem_usage: bool = True
    ) -> Any:
        """
        加载模型 Pipeline
        """
    ...
    def _load_sd_model(model_id: str, low_cpu_mem_usage: bool) -> Any:
        ...
        # 检查 SDXL
        is_sdxl = ("sdxl" in os.path.basename(model_id).lower()) or (
            "xl" in os.path.basename(model_id).lower()
        )

        if (
            model_id.endswith(".safetensors")
            or model_id.endswith(".ckpt")
            or os.path.isfile(model_id)
        ):
                ...
                if os.path.isfile(model_id):
                    ...
                    if not config_file:
                        possible_config = os.path.join(
                            CHECKPOINT_DIR_NEW, "v1-inference.yaml"
                        )
                        if os.path.exists(possible_config):
                            config_file = possible_config
            ...
            if is_sdxl and StableDiffusionXLPipeline is not None:
                logger.info(
                    f"检测到SDXL模型，使用StableDiffusionXLPipeline.from_single_file加载: {model_id}"
                )
                ...
                logger.info(
                    f"使用StableDiffusionPipeline.from_single_file加载: {model_id}"
                )
                kwargs = {
                    "torch_dtype": torch.float16,
                    "low_cpu_mem_usage": low_cpu_mem_usage,
            ...
            if is_sdxl and StableDiffusionXLPipeline is not None:
                logger.info(
                    f"使用StableDiffusionXLPipeline.from_pretrained加载模型: {model_id}"
                )
                ...
                logger.info(
                    f"使用标准StableDiffusionPipeline.from_pretrained加载模型: {model_id}"
                )
