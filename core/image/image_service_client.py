...
class ImageServiceClient:
    ...
    def __init__(self, max_queue_size: int = 10, cache_size: int = 50):
        ...
        self.cache_lock = LazyAsyncLock()  # P2-8: 改用 LazyAsyncLock 避免在 __init__ 中创建 asyncio.Lock（无事件循环时会出错）
    ...
    async def connect(self, ws_url: str = None):
        ...
        try:
            ...
            await self.websocket_client.connect(
                ws_url, service_name="image_client", service_type="image_client"
            )
    ...
    async def _execute_task(self, task: Dict[str, Any]):
        ...
        try:
            if task_type == "generate_image":
                ...
                # 检查 future 是否已被取消（例如调用方已超时放弃）
                if not future.done():
                    future.set_result(result)
            elif task_type == "process_image":
                # 执行图像处理
                result = await self._process_image_impl(
                    task["image_data"], task["file_path"], task["instance_id"]
                )
                if not future.done():
                    future.set_result(result)
        except Exception as e:
    ...
    def _get_prompt_hash(
        ...
        注意：instance_id 必须参与缓存键，否则不同实例（不同模型/LoRA）
        用相同 prompt+尺寸+步数会命中同一个缓存条目，返回错误的结果。
        """
        key = f"{prompt}:{width}:{height}:{num_inference_steps}:{guidance_scale}:{instance_id}"
    ...
    async def _generate_image_impl(
        ...
        """图像生成的实际实现"""
        # 生成缓存键（包含 instance_id 避免不同实例互相污染缓存）
        cache_key = self._get_prompt_hash(
            prompt, width, height, num_inference_steps, guidance_scale, instance_id
        )
        ...
        # 发送请求并等待响应
        response = await self._send_request_and_wait(
            request_id=request_id, request_type="generate_image", data=data
        )
    ...
    async def generate_image(
        ...
        # 等待任务完成，添加整体超时避免 worker 卡死时调用方永久阻塞。
        # request_timeout 已覆盖单次请求耗时，这里 +10s 留给队列等待和实现开销。
        try:
            return await asyncio.wait_for(
                future, timeout=self.request_timeout + 10.0
            )
        except asyncio.TimeoutError:
            if not future.done():
                future.cancel()
            raise RuntimeError(
                f"图像生成任务整体超时（{self.request_timeout + 10.0}s）"
            )
    ...
    async def _process_image_impl(
        ...
        # 发送请求并等待响应
        response = await self._send_request_and_wait(
            request_id=request_id, request_type="process_image", data=data
        )
    ...
    async def process_image(
        ...
        try:
            ...
            if not future.done():
                future.cancel()
            raise RuntimeError(
                f"图像处理任务整体超时（{self.request_timeout + 10.0}s）"
            )
...
async def process_image_async(
    ...
    return await client.process_image(
        image_data=image_data, file_path=file_path, instance_id=instance_id
    )
