...
class PromptProcessor:
    ...
    def optimize_prompt(
        self, prompt: str, category: Optional[ImageCategory] = None
    ) -> str:
        """
        优化提示词以获得更好的生成结果
        ...
        try:
            ...
            if not self._has_quality_guide(optimized):
                quality_modifiers = " ".join(self.positive_modifiers["quality"])[
                    :100
                ]  # 限制长度
            ...
            if category != ImageCategory.OTHER and not self._has_style_guide(optimized):
                ...
                style_modifiers = " ".join(
                    self.positive_modifiers["style"].get(style_type, [])
                )
                if style_modifiers:
    ...
    def generate_negative_prompt(
        self, original_prompt: str, custom_negative: Optional[str] = None
    ) -> str:
        """
        生成负面提示词
        ...
        try:
            ...
            if custom_negative:
                custom_parts = [
                    p.strip() for p in custom_negative.split(",") if p.strip()
                ]
        ...
        try:
            for param_name, limits in self.parameter_limits.items():
                if param_name in params:
                    ...
                    expected_type = (
                        int
                        if param_name in ["width", "height", "num_inference_steps"]
                        else float
                    )
                    ...
                    if value < limits["min"] or value > limits["max"]:
                        return (
                            False,
                            f"参数 {param_name} 必须在 {limits['min']} 到 {limits['max']} 之间",
                        )

            # 特殊检查：宽高比例
            if "width" in params and "height" in params:
                aspect_ratio = max(params["width"], params["height"]) / min(
    ...
    def get_default_parameters(self) -> Dict[str, Any]:
        ...
        """
        return {
            param: limits["default"] for param, limits in self.parameter_limits.items()
        }

    def format_prompt_for_model(
        self, prompt: str, negative_prompt: str, params: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        格式化完整的模型输入
    ...
    def _extract_main_subjects(self, prompt: str) -> List[str]:
        ...
        try:
            ...
            modifiers = set(
                [
                    m.lower()
                    for sublist in self.positive_modifiers.values()
                    for item in (
                        sublist
                        if isinstance(sublist, list)
                        else [v for val in sublist.values() for v in val]
                    )
                    for m in item.split()
                ]
            )
            ...
            subjects = [
                ...
                for word in words
                if len(word.strip(",.!?:;()[]{}'\"-")) > 2
                and word.lower().strip(",.!?:;()[]{}'\"-") not in modifiers
            ]
...
def create_user_friendly_response(
    result: Dict[str, Any], success: bool = True, error_message: Optional[str] = None
) -> Dict[str, Any]:
    """
    创建用户友好的响应格式
    ...
    response = {
        "success": success,
        # P1-1: 同步函数内使用 time.time 替代 asyncio.get_event_loop().time()
        # 避免在 Python 3.10+ 触发弃用警告，且此场景需 wall clock 时间而非 monotonic
        "timestamp": time.time(),
    }
